...
def analyze_and_summarize_tech_news_with_ai(title, summary_text):
    """تحلیل ماهیت خبر تکنولوژی/علمی و نوشتن خلاصه فارسی"""
    content = clean_html(f"Title: {title}\nSummary: {summary_text}")
    if not content:
        return None

    prompt = (
        f"این متن یک پست از یک رسانه خبری/تکنولوژی است:\n\n{content}\n\n"
        "وظایف شما:\n"
        "۱. آیا این پست یک «خبر واقعی» است؟\n"
        "پاسخ باید NO باشد اگر: راهنمای خرید/کار با محصول، نقد و بررسی (Review)، مقاله نظر شخصی، یا تبلیغات خرید کالا باشد.\n"
        "۲. اگر خبر واقعی است، یک خلاصه کوتاه ۱ یا ۲ جمله‌ای به زبان فارسی روان، جذاب و دقیق بنویسید.\n\n"
...
def main():
    ...
    for entry in raw_articles:
        ...
        if fa_summary is None:
            ...
            print(f"خبر غیرمرتبط رد شد: {entry.title}")
            continue
